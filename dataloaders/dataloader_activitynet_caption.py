...
class Activitynet_Caption_DataLoader(Base_DataLoader):
    ...
    def __init__(
        ...
        Args:
        """
        super(Activitynet_Caption_DataLoader, self).__init__()
        ...
        # filter video_ids with unavailable audio or video
        ...
        for video_id in video_id_list:
            ...
            for sub_id in range(n_caption):
                # 有一些数据的start time end time 有误
                if data_dict['start'][sub_id] >= data_dict['end'][sub_id]-1:
                    continue
                ...
                caption = data_dict['captions'][sub_id]
    ...
    def _get_text(self, video_id, sub_id):
        ...
        for i in range(k):
            ind = r_ind[i]
            # 实现两个版本，一种加concept，一种不加 先不加
            words = [self.SPECIAL_TOKEN["CLS_TOKEN"]]#self.tokenizer.tokenize(data_dict['captions'][ind])
            total_length_with_CLS = self.max_words - 1
            if len(words) > total_length_with_CLS:
                words = words[:total_length_with_CLS]

            start_, end_ = max(data_dict['start'][ind]-self.extend,0), data_dict['end'][ind]+self.extend
            ...
            caption_words = self.tokenizer.tokenize(data_dict['captions'][ind])
