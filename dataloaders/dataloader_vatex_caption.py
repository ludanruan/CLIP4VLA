...
class Vatex_Caption_DataLoader(Base_DataLoader):
    ...
    def __init__(
        ...
        Args:
        """
        super(Vatex_Caption_DataLoader, self).__init__()
        self.csv = pd.read_csv(csv)

        self.caption_dict = pickle.load(open(caption_path,'rb'))
        ...
        video_id_list = [str(itm) for itm in self.csv['video_id'].values]
        ...
        for idx, video_id in enumerate(video_id_list):
            ...
            captions = self.caption_dict[video_id]['captions']
            ...
            for idy, caption in enumerate(captions):
                ...
                if 'train' in csv:# training set
                    self.sentences_dict[len(self.sentences_dict)] = (video_id, caption)
                else:
                    if idy == 0: # val set 1 sentence/video, 
                        self.sentences_dict[len(self.sentences_dict)] = (video_id, caption)
