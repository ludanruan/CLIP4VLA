...
########################video retrieval ######################################
# CUDA_VISIBLE_DEVICES=4,5,6,7 python -m torch.distributed.launch --nproc_per_node=4 --master_port=25502 \
...
# --output_dir ${OUTPUT_ROOT}/ckpt_audiocaps_audio_retrieval/feat_fuse \
# --datatype ${DATATYPE}  \
# --lr 2e-7 --max_words 32 --max_frames 12 --batch_size_val 64 \
...
CUDA_VISIBLE_DEVICES=4,5,6,7 python -m torch.distributed.launch --nproc_per_node=4 --master_port=25504 \
main_task_audio_retrieval.py --do_train  --num_thread_reader=4   \
--epochs=15 --batch_size=32 --n_display=100  --filter_video_id --with_control_token 1 \
--model_type audioclip --audio_model audio-clip --cross_model cross-clip \
--train_csv ${TRAIN_CSV} --pretrained_clip_name ViT-B/32 \
--init_model ${INIT_MODEL} \
--val_csv ${VAL_CSV} \
