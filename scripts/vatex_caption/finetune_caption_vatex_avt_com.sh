...
CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=4 --master_port=25413 \
main_task_video_caption.py \
--do_train  --model_type=audioclip  --pretrained_clip_name ViT-B/32 --num_thread_reader=4 \
--epochs=10 --batch_size=64  --audio_model audio-clip --cross_model cross-clip  --with_control_token 0.5 \
...
--output_dir ${OUTPUT_ROOT}/ckpt_vatex_video_caption/pre_compare \
