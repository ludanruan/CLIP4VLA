...
CUDA_VISIBLE_DEVICES=0 python -m torch.distributed.launch --nproc_per_node=1 --master_port=25104 \
main_task_video_retrieval.py --do_train --num_thread_reader=4  \
--epochs=10 --batch_size=32 --n_display=100 --retrieval_finetune loose_seq \
--with_control_token 3 --init_model ${INIT_MODEL}  \
...
--loss_func tav_nce   \
--max_audio_length=16   --train_sim_after_cross \