...
def train_epoch(epoch, args, model, train_dataloader, tokenizer, device, n_gpu, optimizer, scheduler,
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        # input_ids, input_mask, segment_ids, _, _ \
        # pairs_masked_text, pairs_token_labels, _, _\
        # pairs_input_caption_ids, pairs_decoder_mask, pairs_output_caption_ids, \
        # audio, audio_mask, masked_audio, audio_labels_index = batch
        input_dict = {'input_ids':batch[0],  'attention_mask': batch[1], \
                      'pairs_masked_text':batch[5], 'pairs_token_labels': batch[6], \
                        'input_caption_ids': batch[9], 'decoder_mask': batch[10], \
                        'output_caption_ids':batch[11], 'video':batch[3], 'video_mask': batch[4], \
                        'masked_video': batch[7], 'video_labels_index':batch[8]}
...
def collate_active_info(input_tuples, inst_idx_to_position_map, active_inst_idx_list, n_bm, device):
    ...
    sequence_output_rpt, video_output_rpt, input_ids_rpt, input_mask_rpt, video_mask_rpt = input_tuples
    ...
    active_video_output_rpt = collect_active_part(video_output_rpt, active_inst_idx, n_prev_active_inst, n_bm)
    ...
    return (active_sequence_output_rpt, active_video_output_rpt, active_input_ids_rpt, active_input_mask_rpt, active_video_mask_rpt), \
...
def beam_decode_step(decoder, inst_dec_beams, len_dec_seq,
    ...
    def predict_word(next_decoder_ids, n_active_inst, n_bm, device, input_tuples):
        ...
        sequence_output_rpt, video_output_rpt, input_ids_rpt, input_mask_rpt, video_mask_rpt = input_tuples
        '''
        sequence_output, visual_output, audio_output, input_ids, attention_mask, video_mask, audio_mask, input_caption_ids, decoder_mask,
                        shaped=False, get_logits=False
        '''
        input_dict ={'sequence_output':sequence_output_rpt, 'visual_output':video_output_rpt, 'audio_output':None, 'input_ids':input_ids_rpt, \
                    'attention_mask':input_mask_rpt, 'video_mask':video_mask_rpt, 'audio_mask': None, \
                    'input_caption_ids':next_decoder_ids, 'decoder_mask':next_decoder_mask, 'shaped':True, 'get_logits':True}
        dec_output = decoder(**input_dict)
...
def eval_epoch(args, model, test_dataloader, tokenizer, device, n_gpu, nlgEvalObj=None, test_set=None,meta=None):
    ...
    for batch in tqdm(test_dataloader):
        ...
        with torch.no_grad():
            ...
            video_output = model.get_visual_output(video, video_mask)
            # -- Repeat data for beam search
            n_bm = 5 # beam_size
            device = sequence_output.device
            n_inst, len_s, d_h = sequence_output.size()
            _, len_v, v_h = video_output.size()
            ...
            video_output_rpt = video_output.repeat(1, n_bm, 1).view(n_inst * n_bm, len_v, v_h)
            ...
            # -- Prepare beams: length = batchsize
            ...
            for len_dec_seq in range(1, args.max_words + 1):
                active_inst_idx_list = beam_decode_step(decoder, inst_dec_beams,
                                                        ...
                                                        (sequence_output_rpt, video_output_rpt, input_ids_rpt, input_mask_rpt, video_mask_rpt))

                if not active_inst_idx_list:
                    break  # all instances have finished their path to <EOS>

                (sequence_output_rpt, video_output_rpt, input_ids_rpt, input_mask_rpt, video_mask_rpt), \
                inst_idx_to_position_map = collate_active_info((sequence_output_rpt, video_output_rpt, input_ids_rpt, input_mask_rpt, video_mask_rpt),
            ...
            for re_idx, re_list in enumerate(result_list):
                ...
                if args.special_token_dict["CLS_TOKEN"] in decode_text_list:
            ...
            for re_idx, re_list in enumerate(caption_list):
                ...
                if args.special_token_dict["PAD_TOKEN"] in decode_text_list:
                    PAD_index = decode_text_list.index(args.special_token_dict["PADTOKEN"])
                    decode_text_list = decode_text_list[:PAD_index]
                # if decode_text_list[-1] != '.':
                #     decode_text_list.append('.')
                decode_text = ' '.join(decode_text_list)
    ...
    if args.do_train:
        ...
        for epoch in range(args.epochs):
            ...
            if args.local_rank == 0:
                ...
                if epoch > skip_eval:
                    CIDEr = eval_epoch(args, model, test_dataloader, tokenizer, device, n_gpu, nlgEvalObj=nlgEvalObj)
                    if best_score <= CIDEr:
                        best_score = CIDEr
        ...
        if args.local_rank == 0:
            ...
            eval_epoch(args, model, test_dataloader, tokenizer, device, n_gpu, nlgEvalObj=nlgEvalObj, meta=meta)
    elif args.do_eval:
        if args.local_rank == 0:
            eval_epoch(args, model, test_dataloader, tokenizer, device, n_gpu, nlgEvalObj=nlgEvalObj, meta=meta)
