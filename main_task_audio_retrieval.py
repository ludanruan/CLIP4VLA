...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, scaler,local_rank=0):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        input_ids, input_mask, segment_ids, _, _, \
            pairs_masked_text, pairs_token_labels, _, _, audio, audio_mask, masked_audio, audio_labels_index, video_idx = batch
...
def _run_on_single_gpu(model, batch_list_t, batch_list_modal2, batch_sequence_output_list, batch_modal2_output_list , modal1='text', modal2='audio', hard_negative=False):
    sim_matrix = []
...
def eval_epoch_ta(args, model, test_dataloader, device, n_gpu, meta=None):
    ...
    with torch.no_grad():
        batch_list_sequence = []
        batch_list_audio = []
        batch_sequence_output_list, batch_audio_output_list = [], []
        ...
        for bid, batch in enumerate(test_dataloader):
            ...
            input_ids, input_mask, segment_ids, _, _, \
            _, _, _, _, audio, audio_mask, _, _,_ = batch
        ...
        if n_gpu > 1:
            ...
            parameters_tuple_list = [(batch_list_sequence_splits[dev_id], batch_list_audio_splits[dev_id],
                                      batch_sequence_output_splits[dev_id], batch_audio_output_splits[dev_id], 'text', 'audio') for dev_id in device_ids]
            ...
            sim_matrix = _run_on_single_gpu(model, batch_list_sequence, batch_list_audio, batch_sequence_output_list, batch_audio_output_list, 'text', 'audio')
            sim_matrix = np.concatenate(tuple(sim_matrix), axis=0)
            
    print("sim_matrix:",sim_matrix)
    
    metrics = compute_metrics(sim_matrix)
    logger.info('\t Length-T: {}, Length-A:{}'.format(len(sim_matrix), len(sim_matrix[0])))
    ...
    if args.do_train:
        ...
        for epoch in range(args.epochs):
            ...
            if args.local_rank == 0:
                ...
                R1 = eval_epoch_ta(args, model, test_dataloader, device, n_gpu)
        ...
        if args.local_rank == 0:
            ...
            eval_epoch_ta(args, model, test_dataloader, device, n_gpu, meta)
