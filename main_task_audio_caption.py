    ...
    for step, batch in enumerate(train_dataloader):
        ...
        input_dict = {'input_ids':batch[0], 'attention_mask': batch[1], \
                      'pairs_masked_text':batch[5], 'pairs_token_labels': batch[6], \
                        ...
                        'output_caption_ids':batch[11], 'audio':batch[12], 'audio_mask': batch[13], \
...
def collate_active_info(input_tuples, inst_idx_to_position_map, active_inst_idx_list, n_bm, device):
    ...
    sequence_output_rpt, audio_output_rpt, input_ids_rpt, input_mask_rpt, audio_mask_rpt = input_tuples
    ...
    return (active_sequence_output_rpt, active_audio_output_rpt, active_input_ids_rpt, active_input_mask_rpt, active_audio_mask_rpt), \
...
def beam_decode_step(decoder, inst_dec_beams, len_dec_seq,
    ...
    def predict_word(next_decoder_ids, n_active_inst, n_bm, device, input_tuples):
        ...
        '''
        input_dict ={'sequence_output':sequence_output_rpt, 'visual_output':None, 'audio_output':audio_output_rpt, 'input_ids':input_ids_rpt, \
                    'attention_mask':input_mask_rpt, 'video_mask':None, 'audio_mask': audio_mask_rpt, \
        ...
        with torch.no_grad():
            ...
            for len_dec_seq in range(1, args.max_words + 1):
                active_inst_idx_list = beam_decode_step(decoder, inst_dec_beams,
                                                        ...
                                                        (sequence_output_rpt, audio_output_rpt, input_ids_rpt, input_mask_rpt, audio_mask_rpt))

                if not active_inst_idx_list:
                    break  # all instances have finished their path to <EOS>

                (sequence_output_rpt, audio_output_rpt, input_ids_rpt, input_mask_rpt, audio_mask_rpt), \
                inst_idx_to_position_map = collate_active_info((sequence_output_rpt, audio_output_rpt, input_ids_rpt, input_mask_rpt, audio_mask_rpt),
